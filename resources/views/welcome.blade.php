...
<head>
    ...
    <style>
        ...
        /* Scrollbar elegante para la barra lateral (solo aparece si es necesario) */
...
<body class="h-screen w-screen overflow-hidden flex bg-slate-900 font-sans text-white">
    ...
    <div id="toast-container" class="absolute top-20 right-6 z-50 flex flex-col gap-3">
        @if ($errors->any())
            <div
                class="toast-alert bg-red-500/80 backdrop-blur-md border border-red-400 text-white px-6 py-4 rounded-2xl shadow-xl transition-all duration-500">
        ...
        @if(session('success'))
            <div
                class="toast-alert bg-emerald-500/80 backdrop-blur-md border border-emerald-400 text-white px-6 py-4 rounded-2xl shadow-xl font-medium transition-all duration-500">
                {{ session('success') }}
            </div>
        @endif
    </div>
    ...
    <div class="relative z-10 w-full h-full p-6 pt-20 flex gap-6">
        ...
        <aside
            class="w-64 lg:w-80 h-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 flex flex-col shadow-2xl">
                ...
                @forelse($items as $slide)
                    <div data-index="{{ $loop->index }}" onclick="swiper.slideToLoop(this.dataset.index)"
                        ...
                        <div class="w-10 h-10 rounded-lg overflow-hidden bg-black/40 flex-shrink-0">
                            <img src="{{ asset('storage/' . $slide->ruta) }}"
        ...
        <main class="flex-1 h-full flex flex-col items-center justify-between pb-6">
            ...
            <div
                class="w-full max-w-7xl h-[70%] bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2rem] p-4 shadow-2xl">
                    ...
                    <div class="swiper-wrapper">
                        @forelse($items as $slide)
                            <div class="swiper-slide relative flex items-center justify-center">
                                <img src="{{ asset('storage/' . $slide->ruta) }}" class="w-full h-full object-contain">
                                @if($slide->titulo)
                                    <div
                                        class="absolute bottom-6 px-6 py-2 bg-black/60 backdrop-blur-md rounded-full border border-white/10">
            ...
            <div
                class="w-full max-w-2xl mt-6 bg-white/10 backdrop-blur-xl border border-white/20 rounded-[1.5rem] p-6 shadow-2xl">
    ...
    <script>
        // Configuración del Carrusel (Usamos const para que sea accesible globalmente por el onclick)
        ...
        // Esconder Alertas (Toasts) suavemente después de 4 segundos
        setTimeout(() => {
            const toasts = document.querySelectorAll('.toast-alert');
            toasts.forEach(toast => {
                toast.style.opacity = '0';
                toast.style.transform = 'translateY(-20px)';
                setTimeout(() => toast.remove(), 500); // Borrado del DOM
            });
        }, 4000);
        ...
        // Función para aplicar la paleta
